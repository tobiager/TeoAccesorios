...
namespace TeoAccesorios.Desktop
{
    ...
    {
        TextBox txtUser = new TextBox(){ PlaceholderText = "Usuario" };
        TextBox txtPass = new TextBox(){ PlaceholderText = "Contraseña", UseSystemPasswordChar = true };
        Button btnLogin = new Button(){ Text = "Ingresar" };
        ...
        public LoginForm()
        {
            ...
            var formGrid = new TableLayoutPanel{
                Dock = DockStyle.Fill, ColumnCount = 2, RowCount = 3, Padding = new Padding(4)
            };
        ...
        private void stylizeInput(TextBox tb)
        {
        ...
        private void stylizeButton(Button b)
        {
            b.Height = 34;
        ...
        {
            // Demo sin validaciones: deja pasar SIEMPRE con el rol detectado por nombre.
            AuthService.Login(txtUser.Text ?? string.Empty, txtPass.Text ?? string.Empty, out var rol);
            Sesion.Usuario = string.IsNullOrWhiteSpace(txtUser.Text) ? "Invitado" : txtUser.Text.Trim();
            Sesion.Rol = rol;
            Hide();
